...
public class TerraJavaClient {
	...
	public Boolean connect(String pUserType) throws GlueServerException {
		...
		try {
			if (terraJava == null) {
				database = /*dbControll.getDatabase();*/DatabaseConnection.getDatabase(pUserType);
				terraJava = new TerraJavaJNI();
			}
			terraJava.connect(database.getHost(), database.getUser(),
					database.getPassword(), database.getDatabase(),
	...
	public Box convertBox(Box box, String currentProjName,
		...
		HashMap<String, Object> currentProj = null;
		HashMap<String, Object> destinationProj = null;
		...
		if (!currentProjName.equals("LatLong")
				&& !currentProjName.equals("Google")) {
			System.err.println("Projeï¿½ï¿½o invï¿½lida");
		}
		if (!destinationProjName.equals("LatLong")
				&& !destinationProjName.equals("Google")) {
			System.err.println("Projeï¿½ï¿½o invï¿½lida");
		} else if (currentProjName.equals("LatLong")
				&& destinationProjName.equals("Google")) {
			currentProj = getLatLongProj();
			destinationProj = getGoogleProj();
		} else if (currentProjName.equals("Google")
				&& destinationProjName.equals("LatLong")) {
			currentProj = getGoogleProj();
			destinationProj = getLatLongProj();
		}
			...
			if (result) {
				...
				if (nNewViewThemes > 0) {
					...
					for (int i = 0; i < nNewViewThemes; i++) {
						...
						themesIdsVec.add(Integer.parseInt(theme.getId().toString()));
					}
			...
			// If its a new or existing theme, configure the theme
				...
				if (oldTheme == null || (oldTheme.getId() < 0l && persist)) {
					...
					if (persist) {
						...
						if(newTheme.getType() == ThemeType.NETWORK_THEME.ordinal()){
							return true;
						}
				...
				ScaleLimit scaleLimit = newTheme.getScaleLimit();
					if ((scaleLimitChanged || persist) && scaleLimit != null) {
						result = terraJava.setThemeScaleLimit(newTheme
								.getScaleLimit().getMin(), newTheme.getScaleLimit()
								.getMax(), persist, sessionId);
					}
				}
				...
				if(newTheme.isIsVisualChanged() != null){
					if ((newTheme.isIsVisualChanged()   || persist)
						...
						result = configureThemeVisual(sessionId, persist, newTheme);
					}
				}
			}
			return result;
	...
	public Box remapCoordinates(Box box, Projection projFrom, Projection projTo)
		...
		try {
			...
			for (int i = 0; i < result.size(); i++) {
				...
				coordsListResult[i].setX((Double) coord.get("x"));
				coordsListResult[i].setY((Double) coord.get("y"));
	...
	private ThemeVisual getDefaultVisual(Integer rep, String sessionId)
		...
		try {
			...
			System.out.println((Integer) themeVisualVec.get(0));
			t.setStyle((Integer) themeVisualVec.get(0));

			System.out.println((Color) themeVisualVec.get(1));
			Color color = ((Color) themeVisualVec.get(1));
			t.setRed(color.getRed());
			t.setGreen(color.getGreen());
			t.setBlue(color.getBlue());

			System.out.println((Integer) themeVisualVec.get(2));
			t.setTransparency((Integer) themeVisualVec.get(2));

			System.out.println((Integer) themeVisualVec.get(3));
			t.setLineWidth((Integer) themeVisualVec.get(3));

			System.out.println((Integer) themeVisualVec.get(4));
			t.setStyleCountour((Integer) themeVisualVec.get(4));

			System.out.println((Color) themeVisualVec.get(5));
			Color color1 = ((Color) themeVisualVec.get(5));
			t.setCountourColorRed(color1.getRed());
			t.setCountourColorGreen(color1.getGreen());
			t.setCountourColorBlue(color1.getBlue());

			System.out.println((Integer) themeVisualVec.get(6));
			t.setCountourTransparency((Integer) themeVisualVec.get(6));

			System.out.println((Integer) themeVisualVec.get(7));
			t.setWidthCountour((Integer) themeVisualVec.get(7));

			System.out.println((Integer) themeVisualVec.get(8));
			t.setPointSize((Integer) themeVisualVec.get(8));

			System.out.println((Integer) themeVisualVec.get(9));
			t.setPtAngle((Integer) themeVisualVec.get(9));

			System.out.println((String) themeVisualVec.get(10));
			t.setFontName((String) themeVisualVec.get(10));

			System.out.println((Boolean) themeVisualVec.get(11));
			t.setBold((Boolean) themeVisualVec.get(11));

			System.out.println((Boolean) themeVisualVec.get(12));
			t.setItalic((Boolean) themeVisualVec.get(12));

			System.out.println((Double) themeVisualVec.get(13));
			t.setVerticalAlign((Double) themeVisualVec.get(13));

			System.out.println((Double) themeVisualVec.get(14));
			t.setHorizontalAlign((Double) themeVisualVec.get(14));

			System.out.println((Integer) themeVisualVec.get(15));
			t.setTabSize((Integer) themeVisualVec.get(15));

			System.out.println((Integer) themeVisualVec.get(16));
			t.setLineSpace((Integer) themeVisualVec.get(16));

			System.out.println((Boolean) themeVisualVec.get(17));
			t.setFixedSize((Boolean) themeVisualVec.get(17));
	...
	private Layer getLayerFromHashMap(HashMap hashMap, List<Layer> layers) {
	...
	private boolean createThematicMap(ContextToGroupMap contextToGroupMap)
		...
		try {
			if (contextToGroupMap.isIntegrity()) {
				...
				Vector legendVec = terraJava.drawGroupSql(
						contextToGroupMap.getAttributeType(), // parsear pra
																// pegar o tipo
																// do atributo
																// (numerico(0)
																// ou
	...
	public Vector<String> getFeaturesWithRestriction(String restriction) throws GlueServerException {
		try {
	...
	public Vector<String> getFeaturesByIds(List<String> listIds) throws GlueServerException {
