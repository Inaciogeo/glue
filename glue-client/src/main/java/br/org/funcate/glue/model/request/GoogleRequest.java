 ...
 * Google server. \brief This class provides an inflow to solicit tiles from
 * Google server. \author Henrique and Ricardo \version 1.0 \date 20/08/2010
 * 
 */
...
//	private static String id;
	 ...
	 */
	static BufferedImage getImage(GoogleEnum googleType,
			int zoomLevel, int tileIndexX, int tileIndexY, Box box) throws JDOMException {
		String url = null;
...
//				   if(id.equals("2"))// id=0 openStreetMap, id = 1 cgi, id = 2 google;
//					   url = node.getChildText("url");
//				}
				//url = url + "=m@110&hl=pl&x=";
				
				url = PropertiesReader.getProperty(GoogleEnum.SATELLITE.toString());
				break;
			case SATELLITE:
				url = PropertiesReader.getProperty(GoogleEnum.SATELLITE.toString());
				break;
			case TERRAIN:
				url = PropertiesReader.getProperty(GoogleEnum.TERRAIN.toString());
				break;
			case STREET:
				url = PropertiesReader.getProperty(GoogleEnum.STREET.toString());
				break;
			case HYBRID:
				BufferedImage tileImage = GoogleRequest.getImage(
						GoogleEnum.SATELLITE, zoomLevel, tileIndexX,
						tileIndexY, box);
				BufferedImage streetsImage = GoogleRequest.getImage(
						GoogleEnum.STREET, zoomLevel, tileIndexX, tileIndexY,
						box);
				Graphics graph = tileImage.getGraphics();
				graph.drawImage(streetsImage, 0, 0, null);
				return tileImage;
			}
			finalUrl = url + (int) (Math.pow(2, zoomLevel) / 2 + tileIndexX)
					+ "&y="
					+ ((int) (Math.pow(2, zoomLevel) / 2 - tileIndexY) - 1)
					+ "&z=" + zoomLevel;
