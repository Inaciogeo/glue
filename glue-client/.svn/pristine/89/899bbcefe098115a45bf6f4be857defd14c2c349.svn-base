		...
		btnSearch.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent arg0) {
				...
				if (searchText != null && searchText != "" && !searchText.isEmpty()) {
					ResultSet rs = SQLService.buildSelect("select object_id from log_lin where nome LIKE '%"
									+ searchText + "'");
					try {
						while (rs.next()) {
					...
					int idx = Math.abs(size / 2);
					 ...
					 AppSingleton.getInstance().getMediator().setCurrentTool(new ZoomAreaTool()); 
					...
					GlueMessageDialog.show("Campo de pequisa deve ser preenchido!", null, 2);
				}
			}
		});
